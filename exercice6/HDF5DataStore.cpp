...
void HDF5DataStore::createMPIInfo()
{
	...
	//
	// TODO
	// With MPI_Info_set(), change the following values:
	// 	striping_factor => 4
	// 	romio_lustre_coll_threshold => 33554432   (32 MB)
...
void HDF5DataStore::set(const char * name, const double * arr, size_t start[2], size_t local_size[2], size_t total_size[2])
{
	...
	//
	// TODO
	// Use a property list of type H5P_DATASET_XFER and change the transfer to use
	// collective IO rather than independent io.
	// Signature of the function:
	// herr_t H5Pset_dxpl_mpio( hid_t dxpl_id, H5FD_mpio_xfer_t xfer_mode ) 
	//
	// Write data
	hid_t xfer_id = H5Pcreate(...);
	H5Pset_dxpl_mpio(...);
	_status = H5Dwrite(dset_id, H5T_NATIVE_DOUBLE, memspace, filespace, ..., arr);
	H5Pclose(...);
...
void HDF5DataStore::set(const char * name, const double * arr, size_t start, size_t local_size, size_t total_size)
{
	...
	H5Pset_dxpl_mpio(xfer_id, H5FD_MPIO_COLLECTIVE);
