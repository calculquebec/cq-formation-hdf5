...
void HDF5DataStore::createMPIInfo()
{
	...
	MPI_Info_set(_info,"striping_factor","4");
	MPI_Info_set(_info,"romio_lustre_coll_threshold","33554432");
}
...
void HDF5DataStore::set(const char * name, const double * arr, size_t start[2], size_t local_size[2], size_t total_size[2])
{
	...
	_status = H5Dwrite(dset_id, H5T_NATIVE_DOUBLE, memspace, filespace, xfer_id, arr);
	H5Pclose(xfer_id);
